  ...
  implements VirtualScrollStrategy
{
  ...
  private _dataLength = Number.MAX_SAFE_INTEGER;
  ...
  private updateContent() {
    ...
    const start = Math.round((renderedOffset + scrollOffset) / this.rowHeight);
    const itemsDisplayed = Math.ceil(viewportSize / this.rowHeight);
    const bufferItems = Math.ceil(itemsDisplayed * this.bufferMultiplier);
    const end = start + itemsDisplayed + bufferItems;
